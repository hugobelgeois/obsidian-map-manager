...
export class MapCanvas {
	 ...
	 * zone type (including "no zone") are matched and repainted, and vision blockers always stop it.
	 * So filling empty ground stays inside a closed perimeter of walls/other zones, while filling an
	 * already-zoned cell replaces that whole zone (and only that zone) with the brush's settings.
	 */
	private fillFrom(startKey: string): void {
		...
		while (queue.length > 0 && count < FILL_LIMIT) {
			...
			for (const n of this.neighborKeys(key)) {
				if (visited.has(n) || this.blockerTypeAt(n) || this.zoneAt(n) !== targetZone) continue;
				visited.add(n);
		...
		if (data.gridType === "square") {
			...
			for (const c of cells) {
				...
				for (const layer of visibleLayers) {
					...
					if (cell?.visionBlocker) this.drawBlockerBadge(ctx, x + cellSize * 0.88, y + cellSize * 0.12, cellSize, cell.visionBlocker);
				}
			}
		} else {
			const orientation = data.gridType === "hex-pointy" ? "pointy" : "flat";
			...
			for (const c of cells) {
				...
				for (const layer of visibleLayers) {
					...
					if (cell?.visionBlocker) this.drawBlockerBadge(ctx, center.x + cellSize * 0.35, center.y - cellSize * 0.55, cellSize, cell.visionBlocker);
				}
			}
		}
	}

	/** Red X for an "opaque" blocker (hides everything beyond); amber dashed X for a "dim" one (still reveals as explored). */
	private drawBlockerBadge(ctx: CanvasRenderingContext2D, x: number, y: number, cellSize: number, type: VisionBlockerType): void {
		const size = Math.max(3, cellSize * 0.14);
		ctx.save();
		ctx.strokeStyle = type === "opaque" ? "#c0392b" : "#d99a2b";
		ctx.lineWidth = Math.max(1, cellSize * 0.03);
		if (type === "dim") ctx.setLineDash([Math.max(1, cellSize * 0.025), Math.max(1, cellSize * 0.025)]);
		ctx.beginPath();
	 ...
	 * Grid type "none" has no visible cells but still uses the hidden square substrate for this.
	 */
	private blockerTypeAt(cellKey: string): VisionBlockerType | undefined {
		const data = this.controller.getData();
		const gridType = data.gridType === "none" ? "square" : data.gridType;
		let result: VisionBlockerType | undefined;
		for (const layer of data.layers) {
			if (!layer.visible) continue;
			const blocker = layer.cellsByGridType[gridType][cellKey]?.visionBlocker;
			if (blocker === "opaque") return "opaque";
			if (blocker === "dim") result = "dim";
		}
		return result;
	}
