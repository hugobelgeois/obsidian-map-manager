 ...
 * site-root-absolute references (see `rewriteAssetPathsForSite`), bakes linked-note content and a
 * frozen copy of `settings.defaultZoneTypes`/`defaultTokenTemplates` into it (see
 * `renderNoteSnapshot` — zone types/token templates are read live from plugin settings, not stored
 * per-map, so the public export needs its own snapshot-time copy), and writes/overwrites the result
 * next to `mapFile` as `<basename>.json` (see `publicSnapshotPath`) — the file
 * `src/view/customScript.ts` fetches client-side (by URL, derived from the same ` ```map ``` ` block
 * reference) once that script is running on the external site (installation/distribution of the
 * script itself is not this plugin's concern — see `src/view/customScript.ts`).
 */
